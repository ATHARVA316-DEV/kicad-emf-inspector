...
from .constants import (
    MU_0, EPSILON_0, C_LIGHT, RHO_CU, T_CU_1OZ,
    ER_FR4_DEFAULT, TAND_FR4_DEFAULT,
    pcb_velocity_factor, pcb_wavelength_mm,
)
...
class EMFieldEstimator:
    ...
    def compute(self, board: PCBBoard,
        ...
        # Composite power proxy: geometric mean of normalised fields.
        # Note: E and B are from independent models so their product is
        # not a true Poynting vector, but it highlights regions where
        # BOTH fields are elevated — the most likely radiation hotspots.
        S = np.sqrt(self._normalize(E) * self._normalize(B))

        # Composite heatmap — weighted sum of normalised fields.
        # Weights: E and B contribute equally (40% each) as the primary
        # indicators; the combined hotspot proxy contributes 20% to
        # boost regions where both fields are simultaneously elevated.
        heatmap = 0.4 * E_norm + 0.4 * B_norm + 0.2 * self._normalize(S)

        # Frequency correction: radiation efficiency scales with frequency.
        # Normalised to 3 GHz (typical upper bound for common PCB signals).
        # Below 3 GHz, heatmap is attenuated; above, capped at 1.0.
    ...
    def _add_trace_contribution(self, XX: np.ndarray, YY: np.ndarray,
        ...
        # Vectorised: compute all sample points at once
        px = trace.start.x + t_vals * (trace.end.x - trace.start.x)  # (S,)
        py = trace.start.y + t_vals * (trace.end.y - trace.start.y)  # (S,)

        for k in range(len(t_vals)):
            r_mm = np.sqrt((XX - px[k])**2 + (YY - py[k])**2) + 0.1
            # Distance cutoff: skip negligible contributions beyond 50 mm
            mask = r_mm < 50.0
            if not np.any(mask):
                continue
            r_m = r_mm * 1e-3
            ...
            B[mask] += ((MU_0 * I_A) / (2.0 * math.pi * r_m[mask])
                        * (1.0 + loop_factor))

            # Near-field E = V / d
            if trace.length > 0:
                E[mask] += ((V_drop / (r_m[mask] + 1e-6))
                            * (1.0 + loop_factor * 0.5))
