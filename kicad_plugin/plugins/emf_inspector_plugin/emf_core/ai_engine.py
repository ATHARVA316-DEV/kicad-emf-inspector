...
from .constants import RISK_CRITICAL_THRESHOLD, RISK_HIGH_THRESHOLD, RISK_MEDIUM_THRESHOLD
...
class AIAnalysisEngine:
    ...
    def explain(self, issue: EMIIssue) -> AIExplanation:
        ...
        Uses exact title match first, then fuzzy matching to avoid
        silent fallthrough when issue titles change slightly.
        """
        template = _TEMPLATES.get(issue.title)
        if template is None:
            # Fuzzy match: find closest template title
            matches = get_close_matches(
                issue.title, _TEMPLATES.keys(), n=1, cutoff=0.6)
            if matches:
                template = _TEMPLATES[matches[0]]
            else:
                template = self._generic_template(issue)
    ...
    def generate_summary(self, issues: list[EMIIssue],
        ...
        if emi_score >= RISK_CRITICAL_THRESHOLD:
            overall = "CRITICAL — Immediate redesign required"
            color_tag = "🔴"
        elif emi_score >= RISK_HIGH_THRESHOLD:
            overall = "HIGH RISK — Significant EMI improvements needed"
            color_tag = "🟠"
        elif emi_score >= RISK_MEDIUM_THRESHOLD:
        ...
        top_issues = sorted(issues,
                            ...
                            reverse=True)[:3]
        top_text = "\n".join(
            f"  • {i.title} ({i.severity.name}): "
            f"{i.detail_value or (i.description[:60].rsplit(' ', 1)[0] + '…')}"
            for i in top_issues
        )
