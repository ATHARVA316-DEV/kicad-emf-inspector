...
from .constants import __version__, risk_label as _risk_label_fn
...
class ReportGenerator:
    ...
    def export_html(self, board: PCBBoard, report: EMIReport,
        ...
        for issue in sorted(report.issues,
            ...
            # Escape user content to prevent broken HTML output
            esc_title = html_module.escape(issue.title)
            esc_desc = html_module.escape(issue.description)
            esc_cat = html_module.escape(issue.category.upper())
            esc_physics = html_module.escape(ai_exp.physics_background)
            esc_consequence = html_module.escape(ai_exp.emi_consequence)
            esc_improvement = html_module.escape(ai_exp.estimated_improvement)
            steps_html = "".join(
                f"<li>{html_module.escape(s)}</li>" for s in ai_exp.fix_steps)

            issues_html += f"""
            <div class="issue-card" style="border-left: 4px solid {col};">
              <div class="issue-header">
                <span class="badge" style="background:{col}">{issue.severity.name}</span>
                <span class="issue-title">{esc_title}</span>
                <span class="issue-cat">[{esc_cat}]</span>
                {loc_text}
              </div>
              <p class="issue-desc">{esc_desc}</p>
              <details>
                ...
                <div class="ai-section">
                  ...
                  <ol>{steps_html}</ol>
                  <p class="improvement">
                    <strong>Expected Improvement:</strong> {esc_improvement}
                  </p>
                </div>
              </details>
            </div>"""
...
<body>
  ...
  <footer>
    Generated by EMF Inspector v{__version__} &nbsp;|&nbsp;
