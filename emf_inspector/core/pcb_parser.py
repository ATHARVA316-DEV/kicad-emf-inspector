...
class PCBBoard:
    ...
    @property
    def power_nets(self) -> list[int]:
        return [nid for nid, name in self.nets.items()
                if name.startswith("+") or name.startswith("VCC") or
                name.startswith("VDD") or name.startswith("PWR")]
...
class KiCadPCBParser:
    ...
    def parse_text(self, text: str, source: str = "") -> PCBBoard:
        ...
        self._compute_board_size(board)
        return board
    ...
    def _parse_pad(self, pad_node: list, comp: Component,
        ...
        try:
            ...
            if not at or not size_node:
                return None
            pos = _pt(at)
            # Absolute position = component pos + pad offset
            pos = Point(comp.position.x + pos.x,
    ...
    def _compute_board_size(self, board: PCBBoard):
